			    ...
			    struct cattr *attr, struct dentry *hardlink)
{
	...
	if (ovl_type_merge(dentry->d_parent) && S_ISDIR(newdentry->d_inode->i_mode)) {
				    ...
				    struct dentry *hardlink)
{
	...
	err = -ESTALE;
	if ((upper->d_inode == NULL) || !IS_WHITEOUT(d_inode(upper)))
		goto out_dput2;
...
static int ovl_remove_and_whiteout(struct dentry *dentry, bool is_dir)
{
	...
	if (S_ISDIR(upper->d_inode->i_mode))
...
static bool ovl_can_move(struct dentry *dentry)
{
	return ovl_redirect_dir(dentry->d_sb) ||
		!S_ISDIR(dentry->d_inode->i_mode) || !ovl_type_merge_or_lower(dentry);
}
		      ...
		      unsigned int flags)
{
	...
	bool new_is_dir = S_ISDIR(new->d_inode->i_mode);
